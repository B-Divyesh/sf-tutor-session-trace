...
const database = container?.env?.find(candidate => candidate.name === 'DATABASE_URL');
...
assert(database?.value === 'sqlite:///data/trace.db?mode=rwc&vfs=unix-dotfile', 'Azure Files requires SQLite dot-file locking');
